...
MathJax.Hub.Register.StartupHook("TeX Xy-pic Require",function () {
  ...
  // ä»¥ä¸‹ã¯xypic.ModifierRepositoryã«æ ¼ç´ã•ã‚Œã‚‹ã‚‚ã®
  ...
  // ãƒ¦ãƒ¼ã‚¶å®šç¾©ã•ã‚ŒãŸshape
  AST.Modifier.Shape.CompositeModifiers = AST.Modifier.Subclass({
    /**
     * @param {List[AST.Modifier.Shape.*]} modifiers ãƒ¦ãƒ¼ã‚¶å®šç¾©ã•ã‚ŒãŸmodifierã®ãƒªã‚¹ãƒˆ
     */
    ...
    twocellArrow: memo(function () {
      return or(
        ...
        success("no arrow label").to(function () {
          // TODO ç„¡é§„ãªç©ºæç”»å‡¦ç†ã‚’ãªãã™ã€‚
...
//      }
  ...
  xypic.DrawingContext = MathJax.Object.Subclass({
    ...
    /**
     * shapeã‚’æœ€å‰é¢ã«è¿½åŠ ã™ã‚‹ã€‚
     * @param {xypic.Shape} shape è¿½åŠ ã™ã‚‹å›³å½¢
     */
  ...
  xypic.Shape = MathJax.Object.Subclass({
    ...
    /**
     * Bounding Boxã‚’è¿”ã™ã€‚
     * @returns {xypic.Frame.Rect} Bounding Box (ãªã„å ´åˆã¯undefined)
     */
    // getBoundingBox: function () {}
    
    /**
     * ä¸­èº«ãŒç©ºã§ã‚ã‚‹ã‹ã©ã†ã‹ã‚’è¿”ã™ã€‚
     * @returns {boolean} true:ç©ºã§ã‚ã‚‹ã€false:ç©ºã§ãªã„
     */
    isNone: false
  });
  ...
  xypic.Curve = MathJax.Object.Subclass({
    ...
    drawParallelCurve: function (svg, vshift) {
      ...
      for (i = 0; i <= n; i++) {
        t = i/n;
        ts[i] = t;  // TODO: é«˜é€ŸåŒ–ã€‚ts[i+1]-ts[i]ãŒå®šæ•°ã®å ´åˆã«ç‰¹åŒ–ã—ãŸä½œã‚Šã«ã™ã‚‹ã€‚
        angle = this.angle(t);
        p = this.position(t);
        x = p.x;
        y = p.y;
    ...
    drawParallelDottedCurve: function (svg, spacing, vshift) {
      ...
      if (n >= 0) {
        ...
        for (i = 0; i <= n; i++) {
          var s = hpx + i*sp;
          // TODO: é«˜é€ŸåŒ–ã€‚æ¯Žå›žäºŒåˆ†æŽ¢ç´¢ã›ãšã€ç«¯ã‹ã‚‰ç·šå½¢æŽ¢ç´¢ãƒ»é©ç”¨ã™ã‚‹ã‚ˆã†ã«ã™ã‚‹ã€‚
  ...
  xypic.Curve.Line = MathJax.Object.Subclass({
    ...
    toShape: function (context, object, main, variant) {
      // å¤šé‡ç·šã®å¹…ã€ç‚¹ç·šãƒ»ç ´ç·šã®å¹…ã®åŸºæº–
      var env = context.env;
      var thickness = AST.xypic.thickness;
      var s = this.s;
      var e = this.e;
...
//        segment0.drawFatLine();
  ...
  AST.Place.Intercept.Augment({
    value: function (context) {
      ...
      if (intersec.length === 0) {
        ...
        // TODO: è¤‡æ•°å€‹ã®é–‹å§‹åœ°ç‚¹ã‹ã‚‰æŽ¢ç´¢ã—ã€å°¤ã‚‚ã‚‰ã—ã„è§£ã‚’é¸æŠžã™ã‚‹ã€‚
        var x0 = 0;
        var x1 = 0;
  ...
  AST.ObjectBox.Cir.Augment({
    ...
    toConnectShape: function (context) {
      // TODO: ä½•ã‚‚ã—ãªãã¦ã‚ˆã„ã‹TeXã®å‡ºåŠ›çµæžœã‚’ç¢ºèªã™ã‚‹ã€‚
      var env = context.env;
      env.originalReferencePoint = env.c;
      return xypic.Shape.none;
    }
  ...
  AST.Modifier.Shape.Alphabets.Augment({
    preprocess: function (context, reversedProcessedModifiers) {
      ...
      if (modifier !== undefined) {
        ...
        // TODO å­˜åœ¨ã—ãªã„shapeåãŒæŒ‡å®šã•ã‚ŒãŸã‚‰ã‚¨ãƒ©ãƒ¼ã‚’ç™ºç”Ÿã•ã›ã‚‹ã€‚
      }
    },
    modifyShape: function (context, objectShape, restModifiers) {
      var modifier = xypic.repositories.modifierRepository.get(this.alphabets);
